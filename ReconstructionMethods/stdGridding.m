function stdGridding(Kx,Ky,main,class,scan)
%This is the standard gridding function for the Oakland Toolbox.
...
if length(Kx) == 131
    ...
    for step = 1:length(a)
        ...
        %Next, we interpolate on to the standard Cartesian grid with unit
        %spacing.
        ...
        for i = 1:256
            for j = 1:256
                   ...
                   neighbors = fineVals((a(step)*i)-(a(step)-1):(a(step)*i)+1, (a(step)*j)-(a(step)-1):(a(step)*j)+1);  
                   stdVals(i,j) = sum(sum(neighbors))/((a(step)+1)^2);
            end
        end
        ...
        %This makes the unknown parts of the image black with a binary mask.
        black = makeBowl(256);
        imageMat = imageMat.*black;
        
        %This saves the image.
        binTitle = sprintf('oaklandGridding%sScan%s.bin',class,scan);
