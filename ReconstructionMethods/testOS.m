function testOS(Kx,Ky,main,class,scan, fov)
if str2num(fov) > 10
...
%     end
    ...
    sampX = scaledMat(:,:,1)+128;
    sampY = scaledMat(:,:,2)+32;
    sampV = scaledMat(:,:,3);
    
    
    sampV(131,:)=sampV(131,:)+sampV(130,:);
    sampV(132,:)=sampV(132,:)+sampV(133,:);
    ...
    for step=1:length(a)
        ...
        for i=1:512
            for j=1:512
                ...
                if abs(i-256.5) > 1 && abs(j-256.5) > 1
                    z=fineV((a(step)*i*.5)-(a(step)/4):(a(step)*i*.5),(a(step)*j*.5)-(a(step)/4):(a(step)*j*.5));  
                    stdV(i,j)=sum(sum(z))/(((a(step)/4)+1)^2);
                else
                    z=fineV((a(step)*i*.5)-(a(step)/16):(a(step)*i*.5),(a(step)*j*.5)-(a(step)/16):(a(step)*j*.5));  
                    stdV(i,j)=sum(sum(z))/(((a(step)/16)+1)^2);
                end
            end
        end
...
%         ImageMat=ImageMat.*black;
        ...
        binTitle = sprintf('oaklandTestOS%sScan%s.bin',class,scan);
...
%This opens the data.
    ...
    for step=1:length(a)
        ...
        binTitle = sprintf('testOS%sScan%s.bin',class,scan);
        binTitle = fullfile('/Users/yangxia/Desktop/SimonMiller/OaklandReconstructionToolbox/Images',binTitle);
        fid=fopen(binTitle,'w');
        fwrite(fid,ImageMat,'double');
        fclose(fid);
